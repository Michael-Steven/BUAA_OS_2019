...
int sys_env_destroy(int sysno, u_int envid)
{
	/*
		...
		//env_destroy(curenv);
	*/
	//printf("envid to be destroyed : %08x\n", envid);
	int r;
	struct Env *e;
...
int sys_mem_alloc(int sysno, u_int envid, u_int va, u_int perm)
{
	...
	int ret;
	ret = 0;
	if ((perm & PTE_COW) == PTE_COW || (perm & PTE_V) != PTE_V || va >= UTOP) {
		return -E_INVAL;
	}
	if ((ret = envid2env(envid, &env, 0)) != 0) {
		return ret;
				...
				u_int perm)
{
    ...
    //your code here
	if ((perm & PTE_COW) == PTE_COW || (perm & PTE_V) || srcva >= UTOP || dstva >= UTOP) {
		return -E_INVAL;
	}
	...
	ppage = pa2page(va2pa(srcenv->env_pgdir, round_srcva));
	if ((ret = pgdir_walk(srcenv->env_pgdir, round_srcva, 0, &ppte)) != 0) {
		return ret;
	}
	if (ppte != NULL && (*ppte & PTE_V) == 0 && (perm & PTE_V) != 0) {
		return -E_INVAL;
	}
...
int sys_env_alloc(void)
{
	...
	//printf("DEBUG : e->env_tf.cp0_epc = 0x%x\n", e->env_tf.cp0_epc);
	//printf("DEBUG : e->env_tf.pc = 0x%x\n", e->env_tf.pc);
...
{
    // Your code here
	if (!((dev >= 0x10000000 && dev <= 0x10000020 && dev + len >= 0x10000000 && dev + len <= 0x10000020)
			|| (dev >= 0x13000000 && dev <= 0x13004200 && dev + len >= 0x13000000 && dev + len <= 0x13004200)
			|| (dev >= 0x15000000 && dev <= 0x15000200 && dev + len >= 0x15000000 && dev + len <= 0x15000200))) {
		return -E_INVAL;
	} else {
